...
psi_dat_z0_new[chk,:], ayc_te_dat[chk,:]
...
def te_x():
    ...
    plt.figure()
    plt.plot(ayc_r_dat[chk], ayc_te_dat[chk])
    plt.xlabel('radius')
    plt.ylabel('$T_{e}$', rotation=0)
    plt.title('for time index {}'.format(chk))
...
def te_psi():
    ...
    plt.plot(psi_dat_z0_new[chk,:], ayc_te_dat[chk,:], 'g')
    plt.xlabel('$\Psi$')
    plt.ylabel('$T_{e}$', rotation=0)
    plt.title('for time index {}'.format(chk))
...
verts = []
for i in range(len(tme)):
    a = list(zip(psi_dat_z0_new[i,:], ayc_te_dat[i,:]))
    verts.append(a)
    
zs = tme

poly = PolyCollection(verts, edgecolors = dd('cornflowerblue'))#, 
                      #facecolors = cmap(norm(np.sum(lsn_SIS, axis = 1))))
poly.set_alpha(0.9)
