...
tme_repeat = np.repeat(tme.reshape((-1,1)), len(ayc_te_x), axis=1)

tme_flat = np.ndarray.flatten(tme_repeat)
psi_flat = np.ndarray.flatten(psi_sorted)
Te_flat = np.ndarray.flatten(Te_sorted)

psi_new = np.linspace(np.amin(psi_sorted), np.amax(psi_sorted), len(ayc_te_x))
psi_repeat = np.repeat(psi_new.reshape((1,-1)), len(tme), axis=0)
psi_query = np.ndarray.flatten(psi_repeat)

flat_train = np.array([tme_flat, psi_flat, Te_flat])
df_train = pd.DataFrame(flat_train.T, columns=['time', 'psi', 'T_e'])
df_train = df_train.dropna()

flat_query = np.array([tme_flat, psi_query])
df_query = pd.DataFrame(flat_query.T, columns=['time', 'psi'])

forest = KNN()
forest.fit(df_train.loc[:,['time','psi']], df_train.loc[:,'T_e'])
query_output = forest.predict(df_query)


test = np.reshape(query_output, (114,130))
...
plt.figure()
plt.contourf(tme, psi_new, test.T, 33)
plt.colorbar()
