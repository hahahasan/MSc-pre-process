...
tst_z = coeff[0]*(X1**2) + coeff[1]*(Y1**2) + coeff[2]*X1*Y1
+ coeff[3]*X1 + coeff[4]*Y1 + coeff[5]*X1*0+1
...
def Te_vs_psiN_3d():
    ...
    X, Y = np.meshgrid(psi_N_rng, tme)
    Z = np.array(Te_interp)
    ax.plot_surface(X, Y, Z, cmap="jet", lw=0.5, rstride=1, cstride=1)
#    ax.contour(X, Y, Z, 10, lw=3, cmap="autumn_r", 
...
#               linestyles="solid", offset=-1)
    ...
    Z2 = tst_z
    ax.plot_surface(X, Y, Z, cmap="jet", lw=0.5, rstride=1, cstride=1)
    ax.plot_surface(X, Y, Z2, cmap="autumn_r", lw=0.5, rstride=1, cstride=1)
#    ax.contour(X, Y, Z, 10, lw=3, cmap="autumn_r", 
#               linestyles="solid", offset=-1)
    #ax.contour(X, Y, Z, 10, lw=3, colors="k", linestyles="solid")
