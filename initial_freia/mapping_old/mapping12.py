...
for i in range(len(tme)):
    ...
    # no idea why but seems to give correctiish range
    psi_N.append(psi_N_temp)
psi_N = np.array(psi_N)

#psi_N = []
#for i in range(len(tme)):
#    psi_N_temp = ( (psi_norm[i] - mag_ax_psi[i]) /
#                  (psi_boundary[i] - mag_ax_psi[i]) )
#    psi_N_temp[norm_ind[i]:,] = abs(psi_N_temp[norm_ind[i]:,])
#    # no idea why but seems to give correctiish range
#    psi_N.append(-psi_N_temp + 1)
#psi_N = np.array(psi_N)
