...
Te2 = []
for i in range(len(tme)):
    norm_rng = np.where(np.logical_and(psi_fin[i] >= -1, psi_fin[i] <= 1))
    psi_fin2.append(psi_fin[i][norm_rng])
    Te2.append(Te[i][norm_rng])
psi_fin2 = np.array((psi_fin2))
...
def smooth(n_slice=6, smooth_rep=2):
    for hi in range(0,smooth_rep):
        for k in range(len(tme)):
            tmp = np.array_split(Te2[k], len(Te2[k])/n_slice)
