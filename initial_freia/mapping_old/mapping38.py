...
def same_psi2():   
    ...
    for i in range(len(tme)):
        ...
        if cl < 1:
            cl = 1
        rng = np.linspace(fl, cl, int(200*(cl-fl)) + 1)
...
#    rand = np.random.choice(len(B), len(B)/10)   
#    aa = aa[rand]
#    B = B[rand]
...
#ax5.plot_surface(X1, Y1, pred_y.reshape(X1.shape), cmap="jet", lw=0.5, 
...
def plotly_test():
    ...
    data = [
        go.Surface(z=z1, x=psi_N_rng, y=tme),
